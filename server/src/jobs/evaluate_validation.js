...
const outputReportPath = path.join(
  ...
  "..",
  "generated",
  "model_validation_report.xlsx",
);

console.log(
  "⚡ Đang khởi tạo hệ thống đánh giá chéo mô hình AI (Leave-One-Out Validation)...",
);
...
console.log(
  `📚 Đã nạp dữ liệu: ${allStudents.length} sinh viên, ${subjects.length} môn học.`,
);
...
function getValidScoresCount(student) {
  return Object.values(student.scores).filter(
    (v) => v !== null && v !== undefined && v !== "",
  ).length;
}

const sortedStudents = [...allStudents].sort(
  (a, b) => getValidScoresCount(b) - getValidScoresCount(a),
);
const top100Students = sortedStudents.slice(0, 100);

console.log(
  `🎯 Đã chọn ra 100 sinh viên tích cực có nhiều đầu điểm nhất để làm tập mẫu kiểm thử.`,
);
console.log(
  `💡 Sinh viên nhiều điểm nhất có: ${getValidScoresCount(top100Students[0])}/${subjects.length} môn học.`,
);
console.log(
  `💡 Sinh viên thứ 100 có: ${getValidScoresCount(top100Students[99])}/${subjects.length} môn học.`,
);
...
subjects.forEach((targetSubject, subIndex) => {
  ...
  top100Students.forEach((student) => {
    ...
    if (
      actualScoreVal === null ||
      actualScoreVal === undefined ||
      actualScoreVal === "" ||
      isNaN(parseFloat(actualScoreVal))
    ) {
      return;
    }
    ...
    const trainingStudentsMinusS = allStudents.filter(
      (s) => s.mssv !== student.mssv,
    );

    // Chạy mô hình dự báo
    const model = weightedPrediction(
      prereqs,
      targetSubject,
      trainingStudentsMinusS,
    );
    ...
    if (predictedVal === null) {
      ...
      if (otherScores.length > 0) {
        ...
        const trainScores = trainingStudentsMinusS
          .filter(
            (s) =>
              s.scores[targetSubject] != null && s.scores[targetSubject] !== "",
          )
          .map((s) => parseFloat(s.scores[targetSubject]));
        const trainAvg = trainScores.length
          ? trainScores.reduce((a, b) => a + b, 0) / trainScores.length
          : 7.2;
  ...
  const mae =
    totalTested > 0 ? parseFloat((errorSum / totalTested).toFixed(2)) : 0;
  const accuracy =
    totalTested > 0
      ? parseFloat(Math.max(0, 100 - (mae / 10) * 100).toFixed(1))
      : 0;

  const lowPct =
    totalTested > 0
      ? parseFloat(((lowErrorCount / totalTested) * 100).toFixed(1))
      : 0;
  const midPct =
    totalTested > 0
      ? parseFloat(((midErrorCount / totalTested) * 100).toFixed(1))
      : 0;
  const highPct =
    totalTested > 0
      ? parseFloat(((highErrorCount / totalTested) * 100).toFixed(1))
      : 0;
  ...
  if ((subIndex + 1) % 5 === 0 || subIndex === subjects.length - 1) {
    console.log(
      `⏳ Đã chạy đánh giá chéo thành công cho ${subIndex + 1}/${subjects.length} môn học...`,
    );
  }
});
...
const headers = [
  ...
  [
    ...
    "",
    "",
    "Số lượng SV (Đầu người)",
    "Tỉ lệ phần trăm (%)",
    "Số lượng SV (Đầu người)",
    "Tỉ lệ phần trăm (%)",
    "Số lượng SV (Đầu người)",
    "Tỉ lệ phần trăm (%)",
    "Lệch điểm trung bình (MAE)",
...
try {
  ...
  console.log(
    `📊 Báo cáo đánh giá sai số mô hình chi tiết đã được lưu tại: ${outputReportPath}`,
  );
} catch (err) {
  if (err.code === "EBUSY") {
    ...
    console.log(
      `⚠️ Tệp tin đang mở và bị khóa trong Excel. Đã lưu báo cáo cập nhật mới vào: ${fallbackPath}`,
    );
  } else {
    throw err;
  }
...
console.log("\n🎉 THÀNH CÔNG RỰC RỠ!");
console.log(
  "💡 Dữ liệu thống kê hoàn chỉnh cho cả 34 môn học đã sẵn sàng. Bạn có thể mở tệp này để phân tích độ tin cậy!",
);
